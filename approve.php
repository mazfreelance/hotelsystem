...
if (!empty($_POST['sub'])) {
	...
	$name = $_POST['name'];
	$ic = $_POST['ic'];
	$status = $_POST['NightCount'];
	}
	if ($mysqli_db->connect_error) {
    die("Connection failed: " . $mysqli_db->connect_error);
	} 

	$sql = "UPDATE registration SET status='$status' WHERE ic_number=$ic";
	$sql2 = "UPDATE login SET status='$status' WHERE password=$ic";

	if (($mysqli_db->query($sql) == TRUE)&&($mysqli_db->query($sql2) == TRUE)) {
